...
public class Layout {
    ...
    static public Orientation flat = new Orientation(
            3.0 / 2.0,
            0.0, Math.sqrt(3.0) / 2.0, Math.sqrt(3.0),
            2.0 / 3.0,
            0.0, -1.0 / 3.0, Math.sqrt(3.0) / 3.0,
            0.0);
    ...
    public List<Vector2> polygonCornersV(Hex h, float centerDelta) {
        List<Vector2> vector2s = new ArrayList<>();
        Point center = hexToPixel(h);

        for (int i = 0; i < 6; i++) {
            Point offset = hexCornerOffset(i);
            Vector2 vector2 = new Vector2((float) (center.x +offset.x) + centerDelta, (float) (center.y + offset.y) + centerDelta);
            vector2s.add(vector2);
        }
        return vector2s;
    }

    public float[] polygonVertices(Hex h, float centerDelta) {
        List<Vector2> vector2s = polygonCornersV(h, centerDelta);

        float[] res = new float[vector2s.size() *  2];
